# Generate blog posts sorted by date (newest first)
...
# Get all post files and sort by post ID (descending = newest first)
$postFiles = Get-ChildItem $postsDir -Filter "post_*.html" | Sort-Object Name -Descending
Write-Host "Processing $($postFiles.Count) posts (newest first)..."
...
foreach ($file in $postFiles) {
    ...
    if ($contentEnd -lt 0) { $contentEnd = $postHtml.IndexOf("----------", $contentStart) }
    ...
    # Create individual post page (FULL CONTENT)
    $postPage = $header
    ...
    $postPage | Out-File -FilePath (Join-Path $outputDir $fileName) -Encoding UTF8
...
Write-Host "Done! Created $($allPosts.Count) post pages (newest first) with full content on separate pages."
